...
impl PlannerState {
    ...
    pub fn advance(
        ...
        let active_slot = slot_index(now_unix, reset_at_unix, duration_minutes);
        ...
        let mut state = match previous {
            Some(state)
                if state.version == 1
                    ...
                    && used + 0.5 >= state.last_used_percent =>
            {
                state
            }
            _ => Self::fresh(
        ...
        if active_slot > state.active_slot {
            let old_slot = state.active_slot;
            if active_slot == old_slot + 1
                && now_unix.saturating_sub(state.last_sample_unix) <= ATTRIBUTION_GAP_SECS
            {
                state.actuals[old_slot] =
                    Some((used - state.active_start_used_percent).max(0.0).min(100.0));
            }

            state.active_slot = active_slot;
            state.active_start_used_percent = used;
            redistribute_from(&mut state.plans, active_slot, remaining);
        }

        let today_used = (used - state.active_start_used_percent).max(0.0);
        let today_budget = state.plans[state.active_slot].unwrap_or(0.0).max(0.0);
        let keep_for_today = (today_budget - today_used).max(0.0).min(remaining);
        let future_count = SLOT_COUNT.saturating_sub(state.active_slot + 1);

        if future_count > 0 {
            let future_pool = (remaining - keep_for_today).max(0.0);
...
mod tests {
    ...
    #[test]
    fn reset_change_starts_a_new_plan() {
        let reset = 7 * DAY;
        let (state, _) = PlannerState::advance(None, 60, reset, WEEK_MINUTES, 50.0);
        let (_, view) = PlannerState::advance(
            Some(state),
            reset + 60,
            reset + 7 * DAY,
            WEEK_MINUTES,
